...
class Gebruiker(User):
    def __init__(self, id, cash, data_utility, random_factor = None):
        ...
        self.random_factor = random_factor if random_factor is not None else random.uniform(1, 2)
        
    def aciviteit_utility(self):    
        return self.random_factor * math.log(1 + 2 * self.tokens + 1 * self.cash + (self.tokens * token.get_prijs()) ) # we gaan er vanuit dat tokens een grotere invloed hebben op het meedoen van activiteiten

class Speculator(User):
    def __init__(self, id, cash, koop_threshold=None, verkoop_threshold=None):
        super().__init__(id, cash)
    ...
    def koop_utility(self, tokens=None, cash=None, prijs=None):
        if tokens is None:
            tokens = self.tokens
        if cash is None:
            cash = self.cash
        ...
        return math.log(1 + 1 * tokens + 2 * cash + 0.5 * prijs)  # Prijs heeft een lichte invloed
    
    def verkoop_utility(self, tokens=None, cash=None, prijs=None):
        if tokens is None:
            tokens = self.tokens
        if cash is None:
            cash = self.cash
        if prijs is None:
            prijs = self.tokens * token.get_prijs()
        return math.log(1 + 2 * tokens + 1 * cash + 0.5 * prijs)  # Prijs heeft een lichte invloed
    
    def bepaal_aantal_tokens_om_te_handelen(self):
        koop_utility = self.koop_utility()
        verkoop_utility = self.verkoop_utility()

        if koop_utility > verkoop_utility:
            # Bereken hoeveel tokens nodig zijn om het verschil te overbruggen
            delta_tokens = 0
            while self.verkoop_utility(self.tokens + delta_tokens) < koop_utility:
                delta_tokens += 1  # Verhoog de tokens met kleine stappen om te verfijnen
            return delta_tokens
        elif koop_utility < verkoop_utility:
            # Bereken hoeveel tokens nodig zijn om het verschil te overbruggen
            delta_tokens = 0
            while self.koop_utility(self.tokens - delta_tokens) < verkoop_utility:
...
class InvestorGroup:
    ...
    def verkoop_utility(self, prijs_per_token):
        return math.log(1 + 2 * self.vrijgegeven_tokens + 1 * prijs_per_token)
...
class StandaardActiviteit(Activiteiten):
    ...
    def deelname_activiteit(self, token, exchange, gebruiker, hb):
        ...
        if gebruiker.aciviteit_utility() > threshold:
...
class HostActiviteit(Activiteiten):
    ...
    def deelname_activiteit(self, gebruiker, brand):
        # Gebruik de dynamisch berekende drempelwaarde
...
class DataPool(Activiteiten):
     ...
     def deelname_activiteit(self, gebruiker, datapartner):
         # Gebruik de dynamisch berekende drempelwaarde
         threshold = self.bereken_threshold(exchange)
...
class Exchange:
    ...
    def voeg_tokens_toe(self, bron, aantal_tokens, token):       
        ...
        exchange.tokens_op_markt += aantal_tokens
...
if st.button("Start Simulatie"):
    ...
    st.line_chart(FaF.vrijgave_per_iteratie)
